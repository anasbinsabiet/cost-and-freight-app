 $(document).ready(function() {
    ...
    $('#reservation').daterangepicker();
     ...
     $('#display_setting').change(function() {
         if ($(this).val() == 1) {
             ...
             $.each(selectValues, function(key, value) {
                 $('#restriction_type').append($("<option></option>").attr("value", value).text(value));
             });
         }
     });
     ...
     if (payment_method_val == 'bKash') toggleBkashElement(1);
     else toggleBkashElement(0);
     $('#payment_method').change(function() {
         //Use $option (with the "$") to see that the variable is a jQuery object
         var option = $(this).val();
         if (option == 'bKash') toggleBkashElement(1);
         else toggleBkashElement(0);
     });
     $('form').on('keyup', '.product_price', function(e) {
         calculateTotal();
     });
     ...
     $("#table_name").change(function() {
         ...
         $.post(base_url + 'rets/ajax_map_fields', 'table=' + table_name + '&id=' + id).done(function(d) {
             notify.close();
     ...
     $("#field_map").on('click', "input:checkbox", function() {
         ...
         if ($(this).is(":checked")) {
             ...
             $.post(base_url + 'rets/update_field', data).done(function(d) {
             ...
             $.post(base_url + 'rets/delete_field', 'field_name=' + field_name + '&table=' + table_name + '&id=' + id).done(function(d) {
...
function _confirm(obj, targetFunction, data = '', msg = 'You won\'t be able to revert this!', title = 'Are you sure?', type = 'warning', ) {
    swal({
         title: title,
         text: msg,
         type: type,
